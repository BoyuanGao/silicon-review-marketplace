...
- **GPU 加速：** `scripts/detect-gpu.sh` 自动检测 GPU（CUDA / Metal / VAAPI）并配置 peepshow 视频解码加速。whisper.cpp 自动检测 GPU，无需额外配置。无 GPU 时自动回退 CPU。可通过 `eval $(GPU_TYPE=off bash scripts/detect-gpu.sh)` 强制 CPU 模式
